...
$(document).ready(function () {
            ...
            if (data == 0) {
                ...
                if ($('.overview .cart_item').length == 0) {
                    if ($('.overview .no_products').length == 0) {
                        $('.shoppingcart .overview').append('<div class="item no_products">No products added at the moment</div>');
                    }
    ...
    $(document).on('click', '.plus', function (e) {
        product_id = $(this).data('id');
        $.get("/files/functions/shoppingcart.php?add=" + product_id, function (data) {
            if (data == 1) {
